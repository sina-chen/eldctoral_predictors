...
# simulation results
resStan <- readRDS('~/fit_stan/us_senate_predictable/resStan_us_senate_context1990_2022_cf.RDS')
...
p0_r <- ilogit.rv(logit.rv(election_data$vote2_rep) +
                    postrv$mu_alpha + 
                    postrv$beta2*election_data$cf_score_dem + 
                    postrv$beta3*election_data$cf_score_rep + 
...
saveRDS(b0_summary, "~/results_vis/us_senate_predictable/us_senate_b0_cf.RDS")
saveRDS(exp_b0_summary, "~/results_vis/us_senate_predictable/us_senate_exp_b0_cf.RDS")
