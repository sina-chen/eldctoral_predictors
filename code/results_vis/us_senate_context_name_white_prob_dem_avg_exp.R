...
# simulation results
resStan <- readRDS('~/fit_stan/us_senate_predictable/resStan_us_senate_context1990_2022_name_prob_white_dem.RDS')
...
p0_r <- ilogit.rv(logit.rv(election_data$vote2_rep) +
                    postrv$mu_alpha + 
                    postrv$beta2*election_data$name_prob_white_dem +
...
saveRDS(b0_summary, "~/results_vis/us_senate_predictable/us_senate_b0_minority_name_prob_dem.RDS")


#### Average expected EDB ####

avg_b0 <- rv(length = length(election_data$name_prob_white_dem))
  ...
  b0_sim_r_i <- sapply(
    ...
    function(x) 
      ilogit.rv(logit.rv(x) +
                  postrv$mu_alpha + 
                  postrv$beta2*election_data$name_prob_white_dem[i]) - x)
