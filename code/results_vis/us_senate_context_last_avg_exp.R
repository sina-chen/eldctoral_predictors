...
#   average expected edb last poll margin
#
...
avg_b0_last <- rv(length = length( election_data$last_poll_margin))
  ...
  b0_sim_r_i <- sapply(election_data$vote2_rep, 
                       function(x) 
                         ilogit.rv(logit.rv(x) +
                                     postrv$mu_alpha + 
                                     postrv$beta2*election_data$last_poll_margin[i]) - 
                         x)
...
# save
saveRDS(avg_b0_last_summary, "~/results_vis/us_senate_predictable/us_senate_avg_exp_b0_last.RDS")
