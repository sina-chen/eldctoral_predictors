...
# simulation results
resStan <- readRDS('~/fit_stan/us_senate_predictable/resStan_us_senate_context1990_2022_gender.RDS')
...
p0_r <- ilogit.rv(logit.rv(election_data$vote2_rep) +
                    postrv$mu_alpha + 
                    postrv$beta2*election_data$gender_dem +
                    postrv$beta3*election_data$gender_rep +
                    postrv$beta4*election_data$gender_dem*election_data$gender_rep +
...
avg_b0_dem_summary <- mean(exp_b0_r[which(election_data$gender_dem == 1 &
                                        election_data$gender_rep == 0)]) %>%  
  summary() %>% 
  mutate(feature = "female_dem")

avg_b0_rep_summary <- mean(exp_b0_r[which(election_data$gender_rep == 1 &
                                        election_data$gender_dem == 0)]) %>%  
  summary() %>% 
  mutate(feature = "female_rep")

avg_b0_both_summary <- mean(exp_b0_r[which(election_data$gender_rep == 1 &
                                        election_data$gender_dem == 1)]) %>%  
  summary() %>% 
  mutate(feature = "female_both")

avg_b0_neither_summary <- mean(exp_b0_r[which(election_data$gender_rep == 0 &
                                         election_data$gender_dem == 0)]) %>%  
  summary() %>% 
  mutate(feature = "female_neither")
...
# save
saveRDS(b0_summary, "~/results_vis/us_senate_predictable/us_senate_b0_gender.RDS")
saveRDS(avg_b0_summary, "~/results_vis/us_senate_predictable/us_senate_avg_b0_gender.RDS")
