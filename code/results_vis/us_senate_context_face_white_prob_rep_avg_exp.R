...
#   average expected edb face white prob rep
#
...
# simulation results
resStan <- readRDS('~/fit_stan/us_senate_predictable/resStan_us_senate_context1990_2022_face_prob_white_rep.RDS')
...
p0_r <- ilogit.rv(logit.rv(election_data$vote2_rep) +
                    postrv$mu_alpha + 
                    postrv$beta2*election_data$face_prob_white_rep +
...
saveRDS(b0_summary, "~/results_vis/us_senate_predictable/us_senate_b0_minority_face_prob_rep.RDS")


#### Average expected EDB ####


avg_b0 <- rv(length = length(election_data$v))
  ...
  b0_sim_r_i <- sapply(
    ...
    function(x) 
      ilogit.rv(logit.rv(x) +
                  postrv$mu_alpha + 
                  postrv$beta2*election_data$face_prob_white_rep[i]) - x)
...
# save
saveRDS(avg_b0_summary, "~/results_vis/us_senate_predictable/us_senate_avg_exp_b0_face_prob_white_rep.RDS")
