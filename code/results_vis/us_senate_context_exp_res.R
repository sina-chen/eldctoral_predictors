...
# simulation results
resStan <- readRDS('~/fit_stan/us_senate_predictable/resStan_us_senate_context1990_2022_exp.RDS')
...
polls <- polls %>%
  filter(dte < 1460 & !is.na(exp_pc_rep) & !is.na(exp_pc_dem)) %>% # exclude polls conducted more than 4 years (max time to previous election)
...
# Election-level data 
election_data <- polls %>%
  group_by(state_year, state_year_int, cycle, state, vote2_rep, exp_pc_rep, 
           exp_pc_dem) %>%
  summarise() %>%
  ungroup()
...
ex_sd_r <- sqrt(postrv$mu_phi2 + postrv$phi2_sc*postrv$sig_phi2 + 
                  postrv$gamma*log(election_data$exp_pc_dem + election_data$exp_pc_rep))
...
# save
saveRDS(summary_ex_sd_r, "~/results_vis/us_senate_predictable/us_senate_ex_sd_exp.RDS")
saveRDS(summary_exp_ex_sd_r, "~/results_vis/us_senate_predictable/us_senate_exp_ex_sd_exp.RDS")
