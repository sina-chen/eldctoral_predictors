#-------------------------------------------------------------------------------
# 
# Electoral predictors - Merge US senate polls with raising and spending data 1990 -2020
#
# Author: Sina Chen
# Source: FEC
#
...
senate_exp1990_2022 <- readRDS("data/election_features/senate_exp1990_2022.RDS")
...
polls_name <- polls_name %>% 
  mutate(
    last_name = case_when(last_name == "lundergan grimes" ~ "grimes",
                          ...
                          last_name == "vandam" ~ "van dam",
                          last_name == "vanhollen" ~ "van hollen",
                          TRUE ~ last_name),
...
raising_dem <- senate_raising1990_2022 %>% 
  subset(party == "DEM") %>% 
  mutate(cand_name_raising = str_remove_all(
    tolower(candidate_name), 
    ' ii+| [a-z] [a-z]{2}| jr| $|^ |[.]| iv| [a-z]$| [a-z] $'),
...
# check
check <- exp_name_match_dem[with(exp_name_match_dem, 
                                 which(last_name.x != last_name.y)),]
exp_name_match_dem <- exp_name_match_dem[-with(exp_name_match_dem, 
                                               which(last_name.x != last_name.y)),]
...
# check
check <- raising_name_match_dem[with(raising_name_match_dem, 
                                     which(last_name.x != last_name.y)),]
raising_name_match_dem <- raising_name_match_dem[-with(raising_name_match_dem, 
                                                       which(last_name.x != last_name.y)),]
...
saveRDS(polls_dem_raising, "data/polls/us_senate_1990_2022_finance.RDS")
