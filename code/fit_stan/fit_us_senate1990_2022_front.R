#-------------------------------------------------------------------------------
#
# Electoral predictors - front runner model
#
# Author: Sina Chen
#
#-------------------------------------------------------------------------------
...
# election-level data 
vote_sy <- polls %>%
  group_by(state_year, cycle,  state, vote2_rep, front_dem, front_rep) %>%
  summarise() %>%
  ungroup()
...
stan_dat <- list(
  ...
  feature_dem = vote_sy$front_dem,             # election-level feature Dem.
  feature_rep = vote_sy$front_rep,             # election-level feature Rep.
...
saveRDS(resStan, '~/fit_stan/us_senate_predictable/resStan_us_senate_context1990_2022_front3.RDS') 
