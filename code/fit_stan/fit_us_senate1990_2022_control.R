#-------------------------------------------------------------------------------
#
# Electoral predictors - fit state control model
#
# Author: Sina Chen
#
#-------------------------------------------------------------------------------
...
# election-level data 
vote_sy <- polls %>%
  group_by(state_year, cycle,  state, vote2_rep, control_dem, control_rep) %>%
  summarise() %>%
  ungroup()
...
stan_dat <- list(
  ...
  feature_dem = vote_sy$control_dem,           # election-level feature Dem.
  feature_rep = vote_sy$control_rep,           # election-level feature Rep.
...
saveRDS(resStan, '~/fit_stan/us_senate_predictable/resStan_us_senate_context1990_2022_control.RDS') 
