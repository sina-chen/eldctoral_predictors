#-------------------------------------------------------------------------------
#
# Electoral predictors - name prob white rep
#
#
#-------------------------------------------------------------------------------
...
# Election-level data 
vote_sy <- polls %>%
  group_by(state_year, cycle,  state, vote2_rep, name_prob_white_rep) %>%
  summarise() %>%
  ungroup()
...
stan_dat <- list(
  ...
  feature = vote_sy$name_prob_white_rep,
  t = polls$t_sc,
...
saveRDS(resStan, '~/fit_stan/us_senate_predictable/resStan_us_senate_context1990_2022_name_prob_white_rep.RDS') # xx divergencies, alpha 95, tree 12
