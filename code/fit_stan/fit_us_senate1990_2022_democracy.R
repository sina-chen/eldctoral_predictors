#-------------------------------------------------------------------------------
#
# Electoral predictors - state democracy score
#
#
#-------------------------------------------------------------------------------
...
polls <- polls %>%
  filter(t < 1460 & !is.na(democracy_mcmc)) %>% # exclude polls conducted more than 4 years (max time to previous election)
...
# Election-level data 
vote_sy <- polls %>%
  group_by(state_year, cycle,  state, vote2_rep, democracy_mcmc) %>%
  summarise() %>%
  ungroup()
...
stan_dat <- list(
  ...
  feature = vote_sy$democracy_mcmc,
  t = polls$t_sc,
...
saveRDS(resStan, '~/fit_stan/us_senate_predictable/resStan_us_senate_context1990_2022_democracy.RDS') # xx divergencies, alpha 95, tree 12
