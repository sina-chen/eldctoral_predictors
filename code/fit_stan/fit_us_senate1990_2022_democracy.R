#-------------------------------------------------------------------------------
#
# Electoral predictors - fit state democracy score model
#
# Author: Sina Chen
#
#-------------------------------------------------------------------------------
...
polls <- polls %>%
  filter(t < 101 & !is.na(democracy_mcmc)) %>%    # exclude polls conducted more than 100 days before election day
...
stan_dat <- list(
  ...
  feature = vote_sy$democracy_mcmc,            # election-level feature
...
saveRDS(resStan, '~/fit_stan/us_senate_predictable/resStan_us_senate_context1990_2022_democracy.RDS')
