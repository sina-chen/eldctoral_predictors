#-------------------------------------------------------------------------------
#
# Electoral predictors - pc expenditures model
#
#
#-------------------------------------------------------------------------------
...
polls <- polls %>%
  filter(t < 1460 & !is.na(exp_pc_rep) & !is.na(exp_pc_dem)) %>% # exclude polls conducted more than 4 years (max time to previous election) or missing exp
...
stan_dat <- list(
  ...
  feature = log(vote_sy$exp_pc_dem + vote_sy$exp_pc_rep),
  t = polls$t_sc,
...
saveRDS(resStan, '~/fit_stan/us_senate_predictable/resStan_us_senate_context1990_2022_exp.RDS') # xx divergencies, alpha 95, tree 12
