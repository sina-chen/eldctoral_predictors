#-------------------------------------------------------------------------------
#
# Electoral predictors - fit CF Score Rep. model
#
# Author: Sina Chen
#
#-------------------------------------------------------------------------------
...
polls <- polls %>%
  filter(dte < 101 & !is.na(cf_score_rep)) %>%        # exclude polls conducted more than 100 days before election day
...
stan_dat <- list(
  ...
  feature = vote_sy$cf_score_rep,           # election-level feature
...
saveRDS(resStan, '~/fit_stan/us_senate_predictable/resStan_us_senate_context1990_2022_cf_rep.RDS') 