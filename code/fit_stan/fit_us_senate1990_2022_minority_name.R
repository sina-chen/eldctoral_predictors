#-------------------------------------------------------------------------------
#
# Electoral predictors - minority name
#
# Author: Sina Chen
#
#-------------------------------------------------------------------------------
...
# election-level data 
vote_sy <- polls %>%
  group_by(state_year, cycle,  state, vote2_rep, minority_name_dem, 
           minority_name_rep) %>%
  summarise() %>%
  ungroup()
...
stan_dat <- list(
  ...
  feature_dem = vote_sy$minority_name_dem, # election-level feature Dem.
  feature_rep = vote_sy$minority_name_rep, # election-level feature Rep.
...
saveRDS(resStan, '~/fit_stan/us_senate_predictable/resStan_us_senate_context1990_2022_minority_name.RDS') 
