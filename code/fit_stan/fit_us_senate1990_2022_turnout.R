#-------------------------------------------------------------------------------
#
# Electoral predictors - turnout model
#
#
#-------------------------------------------------------------------------------
...
polls <- polls %>%
  filter(dte < 1460 & !is.na(turnout_vep)) %>% # exclude polls conducted more than 4 years (max time to previous election)
...
# Election-level data 
vote_sy <- polls %>%
  group_by(state_year, cycle,  state, vote2_rep, turnout_vep, year_int) %>%
  summarise() %>%
  ungroup()
...
stan_dat <- list(
  ...
  feature = vote_sy$turnout_vep,              # continuous feature associated with excess variance
  t = polls$t_sc,                             # time tor election
...
resStan <- stan(file = "~/fit_stan/stan_ml/ml_senate_var_context_cont_cycle.stan", 
                data = stan_dat,
                chains = 4, iter = 5000,
...
saveRDS(resStan, '~/fit_stan/us_senate_predictable/resStan_us_senate_context1990_2022_turnout_cycle.RDS') # xx divergencies, alpha 95, tree 12
