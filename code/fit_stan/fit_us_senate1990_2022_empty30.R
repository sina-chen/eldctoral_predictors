#-------------------------------------------------------------------------------
#
#  Electoral predictors - empty model 30 days
#
#
#-------------------------------------------------------------------------------
...
polls <- polls %>%
  filter(dte <= 30) %>% # exclude polls conducted more than 4 years (max time to previous election)
  ...
  mutate(state_year = paste0(state, cycle),
         ...
         t_sc = as.numeric(dte)/max(as.numeric(dte))) # 3563 polls
...
saveRDS(resStan, '~/fit_stan/us_senate_predictable/resStan_us_senate_context1990_2022_empty30.RDS') # 0 divergencies, alpha 95, tree 12
