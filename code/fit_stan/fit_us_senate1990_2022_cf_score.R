...
polls <- polls %>%
  filter(t < 1460 & !is.na(cf_score_dem) & !is.na(cf_score_rep)) %>% # exclude polls conducted more than 4 years (max time to previous election)
...
# Election-level data 
vote_sy <- polls %>%
  group_by(state_year, cycle,  state, vote2_rep, cf_score_dem, cf_score_rep) %>%
  summarise() %>%
  ungroup()
...
stan_dat <- list(
  ...
  feature_dem = vote_sy$cf_score_dem,
  feature_rep = vote_sy$cf_score_rep,
  t = polls$t_sc,
...
resStan <- stan(file = "~/fit_stan/stan_ml/ml_senate_bias_context_cat.stan", 
                data = stan_dat,
                chains = 4, iter = 5000,
...
saveRDS(resStan, '~/fit_stan/us_senate_predictable/resStan_us_senate_context1990_2022_cf.RDS') # xx divergencies, alpha 95, tree 12
